...
chirp_index = np.array([[124, 60, 96, 112, 24, 108, 48, 100,
                         1, 18, 67, 90, 66, 15, 31, 38, 0,
                        42, 117, 34, 7, 75, 114, 57],
                        [124, 60, 96, 112, 24, 108, 48, 100,
                         84, 18, 120, 106, 102, 68, 76, 30,
                         1, 42, 74, 34, 7, 76, 113, 57]], dtype=scipy.uint8)
    ...
    if d_states[0] == State.S_READ_PAYLOAD or d_states[1] == State.S_READ_PAYLOAD:
        ...
        if d_states[0] != State.S_READ_PAYLOAD and d_states[1] == State.S_READ_PAYLOAD:
            ...
            if len(pld_indexes[1]) < pld_chirps:
                ...
                # if preamble_indexes[0] < preamble_indexes[1]:
                #
                # else:
        ...
        if d_states[0] == State.S_READ_PAYLOAD and d_states[1] == State.S_READ_PAYLOAD:
            ...
            if sfd_bins[0] >= sfd_bins[1]:
                if len(pld_indexes[0]) < pld_chirps:
                    ...
                    chirp_0 = packet_o[bgn_index_0:end_index_0]
                    chirp_index_0, chirp_max_0, chirp_bin_0 = get_fft_bins(chirp_0, d_samples_per_symbol, False, 2)

                    pld_indexes[0].append([chirp_index_0[np.argmax(chirp_max_0)]])
                    pld_bins[0].append(np.max(chirp_max_0))
                ...
                if len(pld_indexes[1]) < pld_chirps:
                    ...
                    if preamble_indexes[0] < preamble_indexes[1]:
                        prev_index = pld_indexes[0][-2][0]
                        next_index = pld_indexes[0][-1][0]
                    else:
                        prev_index = pld_indexes[0][-1][0]

                        if d_states[0] != State.S_STOP:
                            chirp_0 = packet_o[i + shift_samples[0] + d_samples_per_symbol:
                                               i + shift_samples[0] + 2 * d_samples_per_symbol]
                            chirp_index_0, chirp_max_0, chirp_bin_0 = \
                                get_fft_bins(chirp_0, d_samples_per_symbol, False, 2)
                            next_index = chirp_index_0[np.argmax(chirp_max_0)]
                        else:
                    ...
                    end_index_1 = i + shift_samples[1] + d_samples_per_symbol
                    chirp_1 = packet_o[i + shift_samples[1]:i + shift_samples[1] + d_samples_per_symbol]
                    chirp_index_1, chirp_max_1, chirp_bin_1 = get_fft_bins(chirp_1, d_samples_per_symbol, False, 2)
