...
def detect_preamble(idx_stacks, bin_stacks):
    ...
    for idx in range(len(idx_stacks)):
        ...
        for jdx in range(preamble_chirps):
            if abs(pre_idx - idx_stacks[idx].get_i(jdx)) >= preamble_tolerance \
                    or bin_stacks[idx].get_i(jdx) <= bin_threshold:
                pre_found = False
                break
...
def detect_downchirp(bgn_idx, scope):
    ...
    sfd_bgns = [d_samples_per_symbol * 10, d_samples_per_symbol*10 + time_offset]
...
def recover_packet():
    ...
    for idx in range(0, max(detect_chirp_offset - 12, 0)):
        rec_indexes[0].append(pld_indexes[0][idx][np.argmax(pld_bins[0][idx])])
    ...
    for idx in range(max(detect_chirp_offset - 12, 0), min(pld_chirps, detect_chirp_offset)):
        ...
        for jdx in range(len(pld_indexes[0][idx])):
            if abs(pld_indexes[0][idx][jdx] - preamble_index) >= pld_tolerance \
                    and pld_bins[0][idx][jdx] > bin_threshold:
                curr_index_0.append(pld_indexes[0][idx][jdx])
                curr_max_0.append(pld_bins[0][idx][jdx])
    ...
    if detect_chirp_offset < pld_chirps:
        rec_indexes[0].append(pld_indexes[0][detect_chirp_offset])
    ...
    for idx in range(min(pld_chirps, detect_chirp_offset+1), pld_chirps):
        if detect_samp_offset >= (d_samples_per_symbol - detect_samp_offset):
            curr_idx = pld_indexes[0][idx][1]
            # curr_idx = [[jdx] for jdx in curr_idx]
            curr_max = pld_bins[0][idx][1]
        else:
            curr_idx = pld_indexes[0][idx][2]
            # curr_idx = [[jdx] for jdx in curr_idx]
            curr_max = pld_bins[0][idx][2]
    ...
    for idx in range(0, max(0, pld_chirps - detect_chirp_offset - 1)):
        if detect_samp_offset >= (d_samples_per_symbol - detect_samp_offset):
            curr_idx = pld_indexes[1][idx][2]
            # curr_idx = [[jdx] for jdx in curr_idx]
            curr_max = pld_bins[1][idx][2]
        else:
            curr_idx = pld_indexes[1][idx][1]
            # curr_idx = [[jdx] for jdx in curr_idx]
            curr_max = pld_bins[1][idx][1]
    ...
    if detect_chirp_offset < pld_chirps:
        rec_indexes[1].append(pld_indexes[1][pld_chirps - detect_chirp_offset - 1])
    ...
    for idx in range(max(pld_chirps - detect_chirp_offset, 0), pld_chirps):
        rec_indexes[1].append(pld_indexes[1][idx][np.argmax(pld_bins[1][idx])])
        rec_bins[1].append(np.max(pld_bins[1][idx]))

    for idx in range(detect_chirp_offset+1, pld_chirps):
        if not (type(rec_indexes[0][idx]) is int):
            curr_idx = []
            oly_idx = []
            for jdx in range(idx-detect_chirp_offset-1, idx-detect_chirp_offset+1):
    ...
    for idx in range(0, pld_chirps - detect_chirp_offset - 1):
        if not (type(rec_indexes[1][idx]) is int):
            curr_idx = []
            oly_idx = []
            for jdx in range(idx+detect_chirp_offset, idx+detect_chirp_offset+2):
    ...
    if detect_chirp_offset < pld_chirps:
        idx = detect_chirp_offset
        if type(rec_indexes[1][0]) is int:
            oly_idx = [(rec_indexes[1][0] - detect_bin_offset + d_number_of_bins) % d_number_of_bins]
            curr_idx = []
            curr_max = []
            for jdx in range(len(pld_indexes[0][idx])):
                is_oly = False
                ...
                if not is_oly:
                    curr_idx.append(rec_indexes[0][idx][jdx])
                    curr_max.append(rec_bins[0][idx][jdx])

            if len(curr_idx) == 1:
                rec_indexes[0][idx] = curr_idx[0]
                rec_bins[0][idx] = curr_max[0]
            else:
                rec_indexes[0][idx] = pld_indexes[0][idx][np.argmax(pld_bins[0][idx])]
                rec_bins[0][idx] = np.max(pld_bins[0][idx])
        else:
            rec_indexes[0][idx] = pld_indexes[0][idx][np.argmax(pld_bins[0][idx])]
            rec_bins[0][idx] = np.max(pld_bins[0][idx])

        idx = pld_chirps - detect_chirp_offset - 1
        if type(rec_indexes[0][pld_chirps - 1]) is int:
            oly_idx = [(rec_indexes[0][pld_chirps - 1] + detect_bin_offset + d_number_of_bins) % d_number_of_bins]
            curr_idx = []
            curr_max = []
            for jdx in range(len(pld_indexes[1][idx])):
                is_oly = False
                ...
                if not is_oly:
                    curr_idx.append(rec_indexes[1][idx][jdx])
                    curr_max.append(rec_bins[1][idx][jdx])

            if len(curr_idx) == 1:
                rec_indexes[1][idx] = curr_idx[0]
                rec_bins[1][idx] = curr_max[0]
            else:
                rec_indexes[1][idx] = pld_indexes[1][idx][np.argmax(pld_bins[1][idx])]
                rec_bins[1][idx] = np.max(pld_bins[1][idx])
        else:
            rec_indexes[1][idx] = pld_indexes[1][idx][np.argmax(pld_bins[1][idx])]
            rec_bins[1][idx] = np.max(pld_bins[1][idx])

    oly_indexes.append(np.array(
        np.mod(np.add(rec_indexes[1], d_number_of_bins - detect_bin_offset), d_number_of_bins), dtype=np.int))
    oly_indexes.append(np.array(
        np.mod(np.add(rec_indexes[0], d_number_of_bins + detect_bin_offset), d_number_of_bins), dtype=np.int))
...
# # sf 7 len 16 pkt 21 43 65 87
packet_chirp = [[14,   0,  7,  31,  29,  24,  27, 10,
                 1, 18, 67, 90, 66, 15, 31, 38,
                 111, 2, 10, 108, 27, 42, 27, 33,
                 106, 17, 64, 80, 91, 109, 103, 18,
                 1, 117, 83, 5, 53, 35, 119, 99,
                 54, 66, 47, 69, 111, 64, 49, 92],
                [14,  0,  7,  31,  29,  24,  27,  10,
                 84, 71, 94, 85, 5, 122, 30, 123,
                 58, 87, 49, 115, 106, 63, 24, 100,
...
#                  84, 18, 120, 106, 102, 68, 76, 30,
#                  58, 2, 66, 19, 92, 67, 65, 81,
#                  63, 17, 46, 78, 42, 71, 46, 13,
#                  84, 117, 78, 57, 39, 112, 26, 93,
#                  60, 66, 19, 58, 79, 103, 42, 95]]
...
while i < (len(packet_o)):
    ...
    # if chirp_offset <= i//d_samples_per_symbol <= chirp_offset+12:
    ...
    if d_states[0] == State.S_READ_PAYLOAD or d_states[1] == State.S_READ_PAYLOAD:
        ...
        if d_states[0] == State.S_READ_PAYLOAD and d_states[1] == State.S_READ_PAYLOAD:
            ...
            if len(pld_indexes[0]) < pld_chirps:
                ...
                # if detect_samp_offset >= (d_samples_per_symbol - detect_samp_offset):
                #     chirp_p0[0:detect_samp_offset] = chirp_0[0:detect_samp_offset]
                #     chirp_b0[d_half_samples_per_symbol:d_samples_per_symbol] \
                #         = chirp_0[d_half_samples_per_symbol:d_samples_per_symbol]
                # else:
                #     chirp_p0[0:d_half_samples_per_symbol] = chirp_0[0:d_half_samples_per_symbol]
                #     chirp_b0[detect_samp_offset:d_samples_per_symbol] = chirp_0[detect_samp_offset:d_samples_per_symbol]
            ...
            if len(pld_indexes[1]) < pld_chirps:
                ...
                # if detect_samp_offset >= (d_samples_per_symbol - detect_samp_offset):
                #     chirp_p1[0:d_half_samples_per_symbol] = chirp_1[0:d_half_samples_per_symbol]
                #     chirp_b1[d_samples_per_symbol - detect_samp_offset:d_samples_per_symbol] = \
                #         chirp_1[d_samples_per_symbol - detect_samp_offset:d_samples_per_symbol]
                # else:
                #     chirp_p1[0:d_samples_per_symbol - detect_samp_offset] \
                #         = chirp_1[0:d_samples_per_symbol - detect_samp_offset]
                #     chirp_b1[d_half_samples_per_symbol:d_samples_per_symbol] = \
                #         chirp_1[d_half_samples_per_symbol:d_samples_per_symbol]
