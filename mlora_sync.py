...
packet_idxes_failed = []
time_offsets_failed = []
preamble_values_failed = []
preamble_energies_failed = []
sfd_values_failed = []
sfd_energies_failed = []


for packet_idx in range(len(packets_shr[0])):
    ...
    # samp_offset = 112*d_decim_factor
    ...
    packet_o = np.concatenate(
        (packet_o, np.add(packet_shr[0][time_offset:], packet_shr[1][:len(packet_shr[1]) - time_offset])))
    ...
    while i < (len(packet_o)):
        ...
        if d_states[0] == State.S_READ_PAYLOAD and d_states[1] == State.S_READ_PAYLOAD:
            ...
            if (sfd_begins[1] - sfd_begins[0]) == time_offset:
                packet_idxes.append(packet_idx)
                time_offsets.append([chirp_offset, samp_offset, time_offset])
                preamble_values.append(preamble_indexes)
                preamble_energies.append(preamble_bins)
                sfd_values.append(sfd_indexes)
                sfd_energies.append(sfd_bins)
            else:
                packet_idxes_failed.append(packet_idx)
                time_offsets_failed.append([chirp_offset, samp_offset, time_offset])
                preamble_values_failed.append(preamble_indexes)
                preamble_energies_failed.append(preamble_bins)
                sfd_values_failed.append(sfd_indexes)
                sfd_energies_failed.append(sfd_bins)
