...
class MLoRa2:
    ...
    def __init__(self, bw, samples_per_second, sf, cr, powers, byte_lens, chirp_lens):
        ...
        self.sfd_bgns_list = []   # save all sfd_cnts
        self.sfd_history = []
        self.sfdbin_history = []
    ...
    def read_packets(self):
        ...
        self.end_position = len(self.packet_o)
    ...
    def decoding_packet(self):
        ...
        while idx < len(self.packet_o):
            ...
            #
            #     self.bgn_position = (self.pre_begins[1] + 2) * self.d_samples_per_symbol
            #
            #     break
            #
            # else:
            ...
            if self.d_states[0] == State.S_SFD_SYNC or self.d_states[1] == State.S_SFD_SYNC:
                ...
                if self.d_states[0] == State.S_SFD_SYNC and self.d_states[1] == State.S_SFD_SYNC:
                    ...
                    if succ_cnt == 2:
                        ...
                        if self.sfd_begins[1] < self.sfd_begins[0]:
                            self.sfd_begins.reverse()
            ...
            #
            #     self.bgn_position = self.sfd_cnts[1] + 2 * self.d_samples_per_symbol
            #
            #     self.init_decoding()
            #
            #     idx += self.d_samples_per_symbol
            ...
            if not (type(self.rec_indexes[0][idx]) is int):
                ...
                if 0 <= oly_pld_b < self.pld_chirps:
                    if type(self.rec_indexes[1][oly_pld_b]) is int:
                        oly_idx.append((self.rec_indexes[1][oly_pld_b] - self.detect_bin_offset) % self.d_number_of_bins)
                else:
            ...
            if not (type(self.rec_indexes[1][idx]) is int):
                ...
                if self.detect_chirp_offset <= oly_pld_a < self.pld_chirps:
                    if type(self.rec_indexes[0][oly_pld_a]) is int:
                        oly_idx.append((self.rec_indexes[0][oly_pld_a] + self.detect_bin_offset) % self.d_number_of_bins)
                else:
    ...
    def get_chirp_error(self):
        ...
        for idx in range(len(self.corr_indexes)):
            errors_list = np.abs(np.subtract(self.packet_chirp[self.packet_index], np.array([self.corr_indexes[idx]] * 2)))
    ...
    def get_byte_error(self):
        for idx in range(len(self.rec_bytes)):
            ...
            for jdx in range(0, len(self.rec_bytes[idx])):
                comp_result_0.append(bin(self.packet_byte[self.packet_index][0][jdx] ^ self.rec_bytes[idx][jdx]).count('1'))
                comp_result_1.append(bin(self.packet_byte[self.packet_index][1][jdx] ^ self.rec_bytes[idx][jdx]).count('1'))
...
if __name__ == "__main__":
    ...
    mlora2 = MLoRa2(test_bw, test_samples_per_second, test_sf, test_cr, test_powers, test_byte_lens, test_chirp_lens)
    mlora2.read_packets()
    ...
    while mlora2.bgn_position < min(len(mlora2.packet_o), mlora2.end_position):
